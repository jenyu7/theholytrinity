...
var gend_per = function(){
    ...
    femtot=0;
    maletot=0;
    //sums up the total amount of males/females
    i=0;
    while(i < fem.length){
	femtot+=fem[i];
	i++;
    ...
    //finds the percentage and rounds it
    femper = Math.round(femtot/tot * 100);
    malesper = Math.round(maletot/tot * 100);

    return "Females: " + femper + "%, Males: " + malesper + "%\n";
}

//finds the mean age
var mean_age = function(){
    //list of the total amount of people in each age
    var total = d.map(
	function(n){
    ...
    //sums up the total amount of people
    tot=0;
    i=0;
    ...
    return Math.round(totage / tot); 
    
}

//finds the number of minors
var num_minors = function(){
    //filters the data so that only those under 18 show
    var minors = d.filter(
	function(n){
	    return parseInt(n["age"]) < 18;
	}
    );
    //list of all total amount of minors in each age 
    var minnum = minors.map(
	function(n){
	    return parseInt(n["total"]);
	}
    );

    //sums up the total amount of minors
    mintot=0;
    i=0;
...
//testing functions
console.log("US Population data in 2000:\n")
console.log(gend_per());
